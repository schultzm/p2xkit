...
class Psearcher:
    ...
    def amplimer_table(self):
        ...
        #         # Reformat stdout from primersearch to a list for parsing
                ...
                amplicon['forward_oligo'] = {'primer_seq': Seq(collapsed_iupac(amplimer.hit_info[1][0]), alphabet = IUPAC.ambiguous_dna),
                                        'strand': amplimer.hit_info[1][2],
                                        'position': int(amplimer.hit_info[1][5]) - 1,
                                        'mismatches': int(amplimer.hit_info[1][7])}
                amplicon['reverse_oligo'] = {'primer_seq': Seq(collapsed_iupac(amplimer.hit_info[2][0]), alphabet = IUPAC.ambiguous_dna).reverse_complement(),
